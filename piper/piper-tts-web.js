...
const _TtsSession = class _TtsSession {
  ...
  async init(allowLocalModels, fallbackStrategy) {
    ...
    try {
      ...
      __privateGet(this, _ort).env.wasm.numThreads = 1; // خيط واحد إجباري: الخيوط المتعددة تتطلب ترويسات عزل (COOP/COEP) لا يوفّرها GitHub Pages، وتفشل التهيئة بدونها
...
setupWasmPaths_fn = async function(fallbackStrategy) {
  ...
  try {
    ...
    if (strategy === "cdn") {
      ...
      __privateGet(this, _ort).env.wasm.wasmPaths = "./piper/";
