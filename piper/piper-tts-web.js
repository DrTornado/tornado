...
setupWasmPaths_fn = async function(fallbackStrategy) {
  ...
  try {
    ...
    if (strategy === "cdn") {
      // معطَّل عمداً: نستخدم نسخة ONNX المدمجة (ort.bundle.min.mjs) التي تحتوي WASM بداخلها — لا جلب خارجي إطلاقاً
      (_b = __privateGet(this, _logger)) == null ? void 0 : _b.call(this, "Using bundled WASM (no external fetch)");
      return;
