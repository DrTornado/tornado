...
# حروفُ الأبجدية الصوتية داخل خانةٍ عربية — علامةُ نصٍّ أتلفته معالجةٌ آلية
#
# بقيت في `abide` ثلاثةُ حقولٍ هكذا: «əˈɪ— النبر على المقطع الثاني: ə‑»
# و«əˈəʊ= مسكن». وفي `thorough` سطرٌ انقطع في منتصف النسخة الصوتية:
# «يختلف عن الأمريكي /ˈθɜː». كُتبت صحيحةً ثم أتلفها تمريرٌ آليّ سابق، وبقيت
# معروضةً على شاشة صاحبها شهوراً — والمدقّق لا يمنع اللاتينيّ إلا حرفين
# متتاليين، وهذه رموزٌ لا حروف.
#
# ومرّت على المراجعة أيضاً: قرأ المراجع بطاقة `abide` ولم يرَها. فما تراه
# العين قد يفوتها، وما تحسبه الآلة لا يفوتها.
#
# والنسخةُ الصوتية بين شَرطتين مقصودةٌ ومفيدة: «وتُنطق /iː/ طويلة» شرحٌ
# صحيح. فتُنزع أوّلاً، ولا يبقى للفحص إلا رمزٌ عارٍ بلا شرطتين — وهو علامةُ
# التلف وحدها. وكانت القاعدة تُنذر على الصحيح فجعلت بطاقتين «ناقصتين»،
# فسدّتا الطابور على ثلاث عشرة كلمةً تنتظر.
IPA_SPAN = re.compile(r"/[^/]{1,40}/")
IPA_IN_ARABIC = re.compile(r"[ɐ-ʯˈˌːθðŋ]")


def ipa_damage(text: str) -> bool:
    """أفي السطر رمزُ نطقٍ عارٍ خارج نسخةٍ صوتية بين شَرطتين؟"""
    return bool(IPA_IN_ARABIC.search(IPA_SPAN.sub(" ", text or "")))
...
def line_faults(c: dict) -> list:
    ...
    def check(path, o):
        ...
        # الصيغة الخاطئة لا تُعرض عاريةً كأنها مثالٌ يُحتذى.
        #
        # `recreational` كانت تعرض «more recreational» بارزةً و«لا تُستعمل»
        # باهتةً تحتها، فيقرأ المتعلّم الخطأ ويحفظه. والمواصفة تقول «صحّح
        # الخطأ الشائع واذكر الصواب وحده» — فمتى قال الشرح إن الصيغة لا
        # تُستعمل وجب أن يحمل المثالُ علامتَي الصواب والخطأ.
        #
        # والعبارة تُطابَق كاملةً لا بكلمة «خطأ» وحدها: «قابل للخطأ» معنى
        # صحيح لا تحذيراً، وتضييقُ القاعدة أولى من أن تُنبّه على السليم.
        wrong_hint = re.search(
            r"لا تُستعمل|لا تقل|خطأ شائع|خطأ إملائيّ|الصيغة الثانية خطأ|"
            r"غير مستعمل|لا يُقال|صيغةٌ خاطئة",
            (exar or "")
        )
        if wrong_hint and isinstance(ex, str) and ex and "✗" not in ex:
            bad.append(f"{path}: الشرح يقول إن الصيغة خاطئة والمثال بلا علامة — "
                       f"اكتب «✓ الصواب   ✗ الخطأ»: «{ex[:34]}»")
...
def entry_faults(word: str, c: dict) -> list:
    ...
    # لا قاعدةَ آليّة للترجمة المبتورة.
    #
    # جرّبتُ واحدة: جملةٌ إنجليزية من ستّ كلمات تقابلها كلمتان عربيّتان.
    # فأمسكت العطل الحقيقيّ — «DNA is a very large molecule.» كانت تُقابل
    # بـ«الحمض النوويّ» وحدها — لكنها صاحت معه على أربع ترجماتٍ سليمة:
    # «أحبطه الخبر» ترجمةٌ صحيحة تامّة لـ«He was discouraged by the news».
    #
    # والعربية أوجز من الإنجليزية بطبعها، فلا نسبةَ طولٍ تفرّق بين الإيجاز
    # والبتر. وحارسٌ يصيح على السليم يُعلّم صاحبه تجاهله، فيسقط يوم يصيح
    # على السقيم. فتُترك هذه لعين قارئها، ويبقى المدقّق على ما يقطع فيه.
...
def faults(word: str, c: dict) -> list:
    ...
    for k in PAIRED:
        for i, x in enumerate(c.get(k) or []):
            ...
            # رموزُ النطق داخل خانةٍ عربية: نصٌّ أتلفته معالجةٌ آلية
            for fld in ("ar", "exAr"):
                v = x.get(fld) or ""
                if ARABIC.search(v) and ipa_damage(v):
                    out.append(f"{k}[{i}].{fld} رموزُ نطقٍ في خانةٍ عربية — "
                               f"نصٌّ متلف: {v[:40]}")
